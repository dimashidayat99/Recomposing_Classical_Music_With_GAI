...
10. Flattening: Finally, each arrays were flattened to vocab size times the sequence length of the data. Note that since there are four arrays, there are 4 vocab sizes that represent the unique value of treble notes, unique value of treble durations, unique value of bass notes, and unique value of bass durations.
...
1. Input Layers: The decoder starts with input layers where this layer receives the concatenated latent variable (finalize note or duration latent distribution). There are two input layers were used for each input which are note and duration latent distribution.
