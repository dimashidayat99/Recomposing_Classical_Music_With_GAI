...
The fusion model used the combination of RNN, VAE and GAN. Each of the architecture is implemented together forming a new model. The main architecture of RNN-VAE-GAN is a GAN model where it consist two main component which are generator and discriminator. In this model, the variant of GAN was used namely MDGAN which stand for Multiple Discrimninator Generative Adversial Network. As its name, there are more than one discriminator in this model. The generator component used the VAE architecture in its architecture and the VAE architecture in the generator component used RNN architecture in the encoder part. The encoder of the generator component start with two input layers, each for notes and durations. Each input layer is connected the reshape layer. Both reshape layers is connected to concatenate layer to combine the input data. The conatenate layers is linearly connected with three GRU layers, a dense layer and leaky ReLu layer. The leaky ReLu layer is then connected with two dense layer which represent two features which are notes and durations. Each of the dense layer is connected linearly to two alternating leaky ReLu and dense layer. The final dense layer in the encoder have two different connection. The first connection where the final dense layer in the encoder will be connected to another two dense layer forming latent variables of z means and z log variance. Later the z means and z log variance will be connected to sampling layer to sampled the latent representation of z. On the other hand, the two final dense layer in encoder for each input (notes and durations) will be combined together using concecatenate layers and connected to the another dense layers. This dense layer will be combined with the sampled latent representation of z for notes and durations using concatenate layer forming the final product of latent representation. Both final latent representation of z for notes and durations will be used as input to the dercorder. The input layers of decorder is connected linearly to the three dense layers and reshape layer. The reshape layer is connected to the softmax activation function which is further connected to the flatten layer.

As mentioned before, the fusion model utilizing the MDGAN architecture where there are more than one discriminator. In this fusion model, there are two discriminator is used. The first discriminitor will discriminate based on notes feature while second disriminator will discriminate based on durations feature. Both discriminator share similar architecture where the architecture begins with input layer which is connected to two sets of dense layer, batch normalizaton dense layer, leaky ReLu layer and dropout layer. Next, the sequence is continue with the dropout layer is connected linearly with the two dense layer. The final layer which is dense layer used sigmoid activation function to do the binary classification which discriminate the generated data from generator is fake or real. 

The concept of this fusion model will be very similar to GAN. Start with the generator to generate new data from stochastic noise of each feature, while discriminator notes distinguish the generated notes whether it is from real or fake and discriminator durations distinguish the generated durations whether it is from real or fake. In the model, the both networks are trained separately but simultaneously where the generator network has no access to real example and only learn from the two discriminators judgement. While discriminators use the real example to make the judgement. Both generator and each discriminator compete with each other. The loss of generator is based on the VAE loss where it consist of addition two component which are reconstruction loss and Kullback-Liebler divergence term (KL term). The reconstruction loss is binary cross entropy while KL term is defined as follow:
...
Since there are two sets in total of latent varaibles for notes and durations features, the total loss of generator is defined as follows:

$$ Loss_{Total} =  Loss_{Vae Notes} + Loss_{Vae Durations}$$

#### Music Generation - RNN-VAE-GAN
After the GAN training, the notes and durations from the generated is extracted. The outputs (notes and durations) will go through top k sampling process where one from top ten notes and one from top five durations will be randomly select. The dataframe was constructed based on the selected notes and durations. Since there two clef, there are two total dataframe which are treble clef dataframe and bass clef dataframe. The dataframes will undergo the inverse label encoding and inverse transformation. The processed generated dataframes will be combine together into music score and stream forming a music.
...
Each transformer block has similar architecture, where it contained multi head self attention connected linearly with dropout layer, layer normalization, feed forward network, another dropout ayer and another layer normalization. The first component in the transformer block is multi head self attention which it allows the model to focus on different parts of the input sequence simultaneously by computing attention scores between different positions of the input. The outputs from different attention heads are concatenated and linearly transformed to retain information from different representation subspaces. The second layer, the dropout is a regularization technique that helps prevent overfitting by randomly setting a fraction of input units to zero during training. This encourages the network to learn more robust features and reduces reliance on specific neurons. Layer normalization normalizes the inputs of each layer to have a mean of zero and a standard deviation of one. This helps stabilize the training process and speeds up convergence. The feed foward network component consists of one or more fully connected layers with an activation function applied to the output of each layer. The feedforward network helps the model capture complex dependencies in the data. Another dropout layer is applied after the feedforward network to further regularize the model and prevent overfitting. Another layer normalization step is applied after the second dropout layer to normalize the outputs of the feedforward network.


#### Music Generation - GPT
The outputs of GPT model are in the form of combined features (notes and durations). This output will go through top k sampling process where one from top ten of probability of combined feature will be randomly select. The dataframe was constructed based on the selected value. Since there two clef, there are two total dataframe which are treble clef dataframe and bass clef dataframe. The dataframes will undergo the inverse label encoding where the numerical representation of data will be transform to categorical data. The dataframe which contained only one features which is the combine features will be splitted into two features for notes and durations. Each notes and durations were extracted from combined features will be later produce a new dataframe. The dataframe will undergo the inverse transformation to transform categorical representation of data to the initial data representation. The processed generated dataframes will be combine together into music score and stream forming a music.
